...
class UserController {
    static async login(req, res, next){
        try {
            ...
            if(!user) throw { name: 'AuthenticationFailed', message: 'invalid email or password' }
            else {
                let validPassword = comparePassword(password, user.password)
                if(!validPassword) throw { name: 'AuthenticationFailed', message: 'invalid email or password'}
                else {
                    const payload = {
                        id: user.id,
                        email: user.email,
    ...
    static async createUser(req, res, next){
        try {
            ...
            const newUser = await User.create({
                email, name, img_url, role, password
            })
